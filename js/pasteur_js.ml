...
let iv =
  [|
     0x48; 0x3b; 0x2e; 0xc4; 0x44; 0x0d; 0x2c; 0xb0; 0x46; 0xf4; 0xd5; 0x76
   ; 0xa1; 0x94; 0x26; 0x0d
  |]
...
let make_path ?key ?ln ?hl ?raw code =
  ...
  | Some key ->
      Fmt.kstr
        (Fut.ok <.> Jstr.of_string)
        "%s%s#%s" (Jstr.to_string code) args (Jstr.to_string key)
  | None ->
      Fmt.kstr (Fut.ok <.> Jstr.of_string) "%s%s" (Jstr.to_string code) args
...
let post () =
  ...
  | Some el -> (
      ...
      match
        ( Form.Data.(find data (Jstr.v "paste"))
        , Form.Data.(find data (Jstr.v "encrypted")) )
      with
      | Some (`String str), Some (`String encrypted)
        when Jstr.equal encrypted on ->
          ...
          let[@warning "-8"] (`Json_web_key key
                               : [ `Buffer of _ | `Json_web_key of _ ]) =
            key
          in
          let* key =
            Fut.return
...
let show () =
  match
    ( Document.find_el_by_id G.document raw
    , Document.find_el_by_id G.document output
    , Document.find_el_by_id G.document encrypted )
  with
  | Some src, Some output, Some encrypted
    when Jv.get (El.to_jv encrypted) "textContent" |> Jv.to_jstr = on -> (
      let subtl = Crypto.subtle Crypto.crypto in
      let key = Uri.fragment (Window.location G.window) in
      let* key =
        Fut.return
          Rresult.(
            Base64.decode key >>= (Json.decode <.> Base64.data_to_binary_jstr))
      in
      let* key =
...
let () =
  Jv.set Jv.global "doPost"
    (Jv.repr (fun () -> Fut.to_promise ~ok:(const Jv.undefined) (post ())));
  Jv.set Jv.global "doShow"
    (Jv.repr (fun () -> Fut.to_promise ~ok:(const Jv.undefined) (show ())))
